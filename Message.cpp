#include "Message.h"
#include <bitset>

//Forward delcaration of non-member method
unsigned char reverse_byte(unsigned char x);

// This takes string calculates parity, adds control characters, etc
Message::Message(std::string data) 
{
	m_dataLen = data.length();
	m_fLen = m_dataLen + 3; // 3 control characters
	m_data = data;

	Frame();
}

// Take the raw data from the stream, remove characters, extract real data
Message::Message(unsigned char* buffer, int len)
{
	m_rawMessage = reinterpret_cast<const char*>(buffer);
	m_fLen = m_rawMessage.length();
	Verify();

	if(m_valid)
		DeFrame();

}

int Message::GetDataLength()
{
	return m_dataLen;
}

int Message::GetFrameLength()
{
	return m_fLen;
}

bool Message::isValid()
{
	return m_valid;
}

std::string Message::GetDataString()
{
	return m_data;
}

std::string Message::GetRawString()
{
	return m_rawMessage;
}

void Message::Frame()
{	
	//Insert SYN characters
	m_rawMessage.insert(0,2,22);

	
	//Insert message length control character
	m_rawMessage.insert(2,1,(char)m_data.length());


	//Maximum of 64 data characters
	for(int i = 0; i < m_data.length(); i++)
	{
		unsigned char newChar = m_data.at(i);

		if(CalculateOddParity(newChar))
			newChar |= 1 << 7; //Set odd parity bit

		m_rawMessage.insert(i+3, 1, reverse_byte(newChar));
	}
}

void Message::DeFrame()
{
	//To get to this point, we know the message (block) is valid
	char message[64];
	char* subString = new char[2];

	for(int i = 3; i < m_rawMessage.length(); i++)
	{
		unsigned char newChar = m_rawMessage.at(i);
		newChar = reverse_byte(newChar);
		newChar &= 0x7F; //Clear parity bit

		//We now have our real character
		subString[0] = (char) newChar;
		subString[1] = 0;
		m_data.append(subString);
	}

}

void Message::Verify()
{
	bool valid = true;

	for(int i = 3; i < sizeof(m_rawMessage); i++)
	{
		unsigned char currentChar = m_rawMessage[i];
		bool receivedParity = currentChar & 0x1;

		currentChar &= 0x1; // Clear parity bit
		bool calculatedParity = CalculateOddParity(currentChar);

		valid &= (receivedParity == calculatedParity);

	}
		m_valid = valid;
}

bool Message::CalculateOddParity(unsigned char byte)
{
	std::bitset<8> bitString = std::bitset<8>((unsigned long) byte);

	//Calculate parity
	bool odd = false;
	for(int j = 0; j < 8; j++)
		if(bitString[j]) odd = !odd;

	return odd;
}

unsigned char reverse_byte(unsigned char x)
{
    static const unsigned char table[] = {
        ,
    };
    return table[x];
}